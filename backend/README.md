...
Everything works with no API keys: the vision AI falls back to a mock (see ../DEPLOY.md for keys and deployment), and NGO calls and Vakh posts are
...
**Vision.** `VISION_PROVIDER=groq` with a free key from console.groq.com (model `qwen/qwen3.8-27b`, up to 3 photos,
JSON mode, thinking switched off). `gemini` and `ollama` also work. If the provider fails, the API falls back to the
mock and says so in `warnings`. The AI never sets prices.

**Photo storage.** `STORAGE_BACKEND=local` saves to `UPLOAD_DIR`; `supabase` uploads to a public Supabase Storage bucket
(needed on Railway, whose disk is wiped on every deploy).
