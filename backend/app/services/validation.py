...
def validate_listing(spec: CategorySpec, data: dict, now: datetime | None = None) -> tuple[dict, list[dict]]:
    ...
    if len((data.get("title") or "").strip()) < 4:
        err("title", "give it a short, searchable title (at least 4 characters)")
    qty = data.get("quantity")
    if qty is None or qty <= 0:
        err("quantity", "enter how much there is")
    ...
    if reason not in reason_keys:
        err("reason_code", "choose why it's being given away or sold")
