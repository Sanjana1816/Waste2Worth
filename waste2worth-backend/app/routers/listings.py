...
def _out(session: Session, li: Listing, lat: float | None = None, lng: float | None = None) -> dict:
    ...
    d["images"] = [{"id": i.id, "shot_type": i.shot_type, "url": f"/uploads/{Path(i.path).relative_to(settings.upload_dir).as_posix()}",
...
def create_listing(body: ListingCreate, session: Session = Depends(get_session)):
    ...
    li = Listing(**data, quantity_available=body.quantity)
...
async def upload_image(listing_id: int, shot_type: str = Form(...), file: UploadFile = File(...),
    ...
    folder = Path(settings.upload_dir) / str(listing_id)
    folder.mkdir(parents=True, exist_ok=True)
    path = folder / f"{shot_type}_{uuid.uuid4().hex[:10]}.jpg"
    img.thumbnail((2048, 2048))
    img.save(path, format="JPEG", quality=88)

    row = ListingImage(listing_id=listing_id, shot_type=shot_type, path=str(path), **result)
    session.add(row)
    session.commit()
    session.refresh(row)
    return {
