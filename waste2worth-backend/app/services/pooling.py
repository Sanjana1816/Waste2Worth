...
def quote(session: Session, req: PoolRequest) -> dict:
    ...
    if req.listing_id:
        ...
        if not spec.poolable:
            raise HTTPException(400, f"{spec.label} can't be pooled")
        key, unit, ref_lab = listing_pool_key(spec, ref), ref.unit, ref.measured_color_lab
    ...
    candidates, excluded = [], []
    for li in rows:
        if listing_pool_key(spec, li) != key or available(li) <= 0 or li.asking_price_per_unit is None:
            continue
    ...
    platform = round(valuation.commission(subtotal, "resell") + (valuation.POOL_FLAT_FEE if stops > 1 else 0), 2)
    ...
    if new_total and total >= 0.9 * new_total:
