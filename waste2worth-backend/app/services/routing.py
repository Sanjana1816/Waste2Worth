...
def recommend(spec: CategorySpec, data: dict) -> dict:
    ...
    if "resell" in spec.routes and cond in ("new", "like_new", "good", "fair"):
        if spec.poolable:
            reasons.append("This category can be pooled: your stock can be combined with other sellers' matching stock for bigger buyers.")
        return pick("resell", "It still has resale value, and a buyer nearby can use it as-is.")
